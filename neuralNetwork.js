...
class NeuralNetwork{
    ...
    train(arr, target){
        ...
        let derivate_output = Matrix.map(output, dsigmoid)
        let hidden_T = Matrix.transpose(hidden);
        
        let gradient = Matrix.hadamard(output_error, derivate_output)
        gradient = Matrix.scalar_multiply(gradient, this.learning_rate)
        ...
        let derivate_hidden = Matrix.map(hidden, dsigmoid)
        let input_T = Matrix.transpose(input)

        let gradient_H = Matrix.hadamard(hidden_error, derivate_hidden)
